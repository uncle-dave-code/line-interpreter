...
import com.dscfgos.interpreter.utils.OperatorsUtils;
...
public class ExpressionParser {
    ...
    public String interprete(String context, List<InterpreteProperty> params) {
        return this.evaluateExpression(context, params);
    }

    /**
     * Rules: 1: Scan from Left to Right 2: If the scanned item is an ‘(‘, push it to the stack. 3: If the scanned item
     * is an ‘)’, pop the stack and output it until a ‘(‘ is encountered, and ignore both parenthesis. 4: If the
     * scanned item is an operator a) If the precedence of operator is greater than the precedence of the operator in the
     * stack or the stack is empty or the stack contains a ‘(‘ , push it. b) Pop all the operators from the stack which
     * are greater than or equal to in precedence than that of the scanned operator. After doing that Push the scanned
     * operator to the stack. (If you encounter parenthesis while popping then stop there and push the scanned operator in
     * the stack. 5: If the scanned item is an operand, output it. 6: Pop and output from the stack until it is not empty
     *
     * @param context Infix expression
     * @return Postfix expression
     */
    public String convertToPostFixExpression(String context, List<InterpreteProperty> params) {
        String output = "";
    ...
    private String evaluateExpression(String expression, List<InterpreteProperty> params) {
